...
type RegisterService interface {
	...
	GetMonthAppointments(ctx context.Context, userId int) error
...
func NewRegisterService(mailer mail.Mailer) RegisterService {
...
func (s *registerService) GetWeekAppointments(ctx context.Context, userId int) ([]domain.DailyRegister, error) {
	...
	weekAppointments := s.mergeIntoDays(appointments)
	weekRegisters := s.mountDailyRegister(weekAppointments)

	return weekRegisters, nil
}

func (s *registerService) GetMonthAppointments(ctx context.Context, userId int) error {
	report := domain.MonthReportRequest{
		UserId: userId,
		Time:   time.Now(),
		Mail:   "", //TODO: get user mail
	}
...
func (s *registerService) ReportAppointments(ctx context.Context, reportRequest domain.MonthReportRequest) error {
	...
	monthAppointments := s.mergeIntoDays(report)
	monthRegisters := s.mountDailyRegister(monthAppointments)
	...
	s.mailer.Send("Month Clock In Report", message, []string{reportRequest.Mail})
